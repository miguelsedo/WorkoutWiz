...
export const TrainingPageEdit = () => {
    ...
    const handleChangeExercise = async (exercise) => {
        ...
        try {
            const response = await axios.post('http://localhost:3001/interact-with-gpt', { prompt });
    ...
    const handleDeleteRoutine = async () => {
        if (window.confirm('Are you sure you want to delete this training plan?')) {
            try {
                const response = await fetch(`http://localhost:3001/delete-routine/${routineId}/${userId}`, {
                    method: 'DELETE'
                });
